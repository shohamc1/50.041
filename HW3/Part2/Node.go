...
func (n *Node) Read(pageID int) string {
	...
	if reqPage.Access == NONE {
		...
		if !n.Manager.up {
			fmt.Println("Manager is not available and election has not finished")
			return ""
		}
		returnData := n.Manager.ReadRequest(pageID, n)
		n.Pages = append(n.Pages, returnData)
...
func (n *Node) UpdateManager(newManager *Manager) {
